...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        ...
        if (documents_.count(document_id)) {
            throw invalid_argument("Документ с id раннее добавленного документа"s);
        }
        if (!IsValidWord(document)) {
            throw invalid_argument("Наличие недопустимых символрв"s);
        }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text.empty()) {
            throw invalid_argument("Нет текста"s);
        }
        bool is_minus = false;
        if (text[0] == '-') {
            is_minus = true;
...
int main() {
    ...
    AddDocument(search_server, -1, "пушистый пёс и модный ошейник"s, DocumentStatus::ACTUAL, { 1, 2 });
    AddDocument(search_server, 3, "большой пёс скво\x12рец"s, DocumentStatus::ACTUAL, { 1, 3, 2 });
    AddDocument(search_server, 4, "большой пёс скворец"s, DocumentStatus::ACTUAL, { 1, 1, 1 });
